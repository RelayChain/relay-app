...
const TransactionLine = ({ mints, burns, swaps }: TransactionLineProps) => {
  ...
  const getData = () => {
    switch (true) {
      case mints.length > 0:
        return {
          type: `Add ${mints[0].pair.token0.symbol} and ${mints[0].pair.token1.symbol}`,
      ...
      case burns.length > 0:
        return {
          type: `Remove ${burns[0].pair.token0.symbol} and ${burns[0].pair.token1.symbol}`,
      ...
      case swaps.length > 0:
        return {
          type: `Swap ${swaps[0].pair.token0.symbol} and ${swaps[0].pair.token1.symbol}`,
  ...
  const { type, totalValue, amount0, amount1, transaction, date } = data
  return (
    <>
      <Flex>
        <Text>{type}</Text>
      ...
      {width < 500 && <InnerWrap onClick={() => setisOpen(!isOpen)} />}
    </>
  )
}
